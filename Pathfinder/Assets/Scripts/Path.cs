...
public class Path
{
    ...
    public Path(Vector3[] waypoints, Vector3 startPos, float turnDistance, float stoppingDistance)
    {
        ...
        Vector2 previousPoint = ConvertVector2(startPos);
        for (int i = 0; i < lookPoints.Length; i++)
        {
            Vector2 currentPoint = ConvertVector2(lookPoints[i]);
            Vector2 dirToCurrentPoint = (currentPoint - previousPoint).normalized;
            //If index i is equal to finish line index turn boundary point is equal to the current point otherwise currentpoint minus direction times distance
    ...
    //COnverting vector 3 coordinates to vector 2 //Due to the nature of the project, y coordinate values are put into the z coordinate

    Vector2 ConvertVector2(Vector3 v3)
    {
