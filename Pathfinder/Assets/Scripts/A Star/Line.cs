...
public struct Line
{
    const float verticalLineGradient = 1e5f; //10^5
    float gradient;
    float y_intercept;
    ...
    public Line(Vector2 pointOnLine, Vector2 pointPerpendicularToLine)
    {
        ...
        //If line is vertical, dx should be zero
        if(dx == 0)
        {
        ...
        {
            gradientPerpendicular = dy / dx; //Slope, rise over run
        }
    ...
    {
        //true if on one side, false if the other
